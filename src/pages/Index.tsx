...
const Index = () => {
  ...
  const handleImageUpload = useCallback((file: File, index: 1 | 2) => {
    ...
    reader.onload = e => {
      ...
      if (index === 1) setImage1(result);else setImage2(result);
      playSound("upload");
    };
  ...
  const handleCropComplete = useCallback((croppedImage: string) => {
    if (cropImage) {
      if (cropImage.index === 1) setImage1(croppedImage);else setImage2(croppedImage);
      setCropImage(null);
  ...
  const handleTransform = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke("generate-snap", {
        body: {
          image1,
          image2,
          ratio: selectedRatio,
      ...
      if (data?.frames && data.frames.length > 0) {
        ...
        const demoFrames = Array.from({
          ...
          const images = ["https://images.unsplash.com/photo-1529156069898-49953e39b3ac?w=400&h=500&fit=crop", "https://images.unsplash.com/photo-1516589178581-6cd7833ae3b2?w=400&h=500&fit=crop"];
  ...
  return <div className="min-h-screen relative overflow-x-hidden" style={{
    background: "linear-gradient(180deg, hsl(250 30% 8%) 0%, hsl(250 25% 5%) 100%)"
      ...
      <GenerationNotification isVisible={showGenerationNotification} userName={user?.email} frameCount={generatedFrames.length} onClose={() => setShowGenerationNotification(false)} onViewResults={() => setShowPreview(true)} />
      ...
      <div className="container max-w-md mx-auto px-4 py-8 md:py-12 relative z-10">
        ...
        <motion.header className="text-center mb-8 relative" initial={{
        opacity: 0,
        y: -30
      }} animate={{
        opacity: 1,
        ...
        duration: 0.8
          ...
          <motion.div className="absolute -top-4 left-1/4 w-2 h-2 rounded-full bg-primary" animate={{
          y: [0, -15, 0],
          opacity: [0.4, 1, 0.4],
          scale: [1, 1.3, 1]
        }} transition={{
          ...
          boxShadow: "0 0 15px hsl(270 95% 65%)"
        }} />
          <motion.div className="absolute top-8 right-1/4 w-1.5 h-1.5 rounded-full bg-secondary" animate={{
          y: [0, -10, 0],
          opacity: [0.3, 0.8, 0.3]
        }} transition={{
          duration: 2.5,
          ...
          boxShadow: "0 0 12px hsl(35 100% 60%)"
        }} />
          <motion.div className="absolute -top-2 right-1/3 w-1 h-1 rounded-full bg-foreground/60" animate={{
          y: [0, -8, 0],
          x: [0, 5, 0]
        }} transition={{
          duration: 4,
          ...
          <motion.h1 className="text-4xl md:text-5xl font-serif font-bold mb-3 flex items-center justify-center gap-2 relative" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} transition={{
          ...
          perspective: "1000px"
            ...
            <motion.span className="text-metallic relative" animate={{
            textShadow: ["0 0 20px hsl(270 95% 65% / 0.3)", "0 0 40px hsl(270 95% 65% / 0.5)", "0 0 20px hsl(270 95% 65% / 0.3)"]
          }} transition={{
            duration: 3,
            repeat: Infinity
          }} whileHover={{
            ...
            <motion.span className="inline-flex items-center justify-center w-12 h-12 rounded-xl text-2xl font-mono relative" style={{
            ...
            <motion.span className="text-metallic" animate={{
            textShadow: ["0 0 20px hsl(35 100% 60% / 0.3)", "0 0 40px hsl(35 100% 60% / 0.5)", "0 0 20px hsl(35 100% 60% / 0.3)"]
          }} transition={{
            duration: 3,
            repeat: Infinity,
            delay: 0.5
        ...
        <AnimatePresence mode="wait">
          {(appState === "upload" || appState === "configure") && <motion.div key="upload-section" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} exit={{
          ...
          scale: 0.95
              ...
              <GlowingBorder colors={["hsl(270 95% 65%)", "hsl(35 100% 60%)", "hsl(300 80% 60%)"]} speed={4}>
                <motion.div className="p-6 rounded-2xl relative overflow-hidden" style={{
              ...
              stiffness: 100
                ...
                ease: "easeInOut"
                  ...
                  perspective: "1000px"
                      ...
                      <PortraitCard label={swapPositions ? "Portrait 2" : "Portrait 1"} image={swapPositions ? image2 : image1} onImageUpload={file => handleImageUpload(file, swapPositions ? 2 : 1)} onRemoveImage={() => swapPositions ? setImage2(null) : setImage1(null)} onCropRequest={() => handleCropRequest(swapPositions ? 2 : 1)} />
                    </motion.div>
                    
                    {/* Connection indicator */}
                    <motion.div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-0" animate={bothImagesUploaded ? {
                  scale: [1, 1.5, 1],
                  opacity: [0.3, 0.8, 0.3]
                } : {
                  opacity: 0
                  ...
                  perspective: "1000px"
                      ...
                      <PortraitCard label={swapPositions ? "Portrait 1" : "Portrait 2"} image={swapPositions ? image1 : image2} onImageUpload={file => handleImageUpload(file, swapPositions ? 1 : 2)} onRemoveImage={() => swapPositions ? setImage1(null) : setImage2(null)} onCropRequest={() => handleCropRequest(swapPositions ? 1 : 2)} />
                    </motion.div>
                  </div>
                </motion.div>
              </GlowingBorder>
              ...
              {/* Controls */}
              <AnimatePresence>
                {bothImagesUploaded && <motion.div initial={{
              opacity: 0,
              height: 0
            }} animate={{
              opacity: 1,
          ...
          {appState === "processing" && <motion.div key="processing" className="flex flex-col items-center justify-center min-h-[60vh]" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} exit={{
          ...
          {appState === "scratch" && generatedFrames.length > 0 && <motion.div key="scratch" className="flex flex-col items-center" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }}>
          ...
          {appState === "result" && <motion.div key="result" className="space-y-8" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }}>
              ...
              <div className="flex flex-col items-center gap-4">
                ...
                playSound("preview");
                setShowPreview(true);
              }} className="px-6 py-3 rounded-xl font-mono text-sm tracking-wider flex items-center gap-2 relative overflow-hidden" style={{
                ...
                {generatedFrames.length > 0 && <SocialShareButtons imageUrl={generatedFrames[0]} caption="Check out my Selfie2Snap creation! Two selfies, one epic moment! 🍌✨" onDownload={() => {
              playSound("download");
              // Trigger download
